...
export function ProductFilters({ categories }: ProductFiltersProps) {
  ...
  // Memoize URL param values to prevent unnecessary re-renders
  ...
  const currentSearch = urlParams.search;
  const currentCategory = urlParams.category;
  const currentColor = urlParams.color;
  const currentMaterial = urlParams.material;
  const currentSort = urlParams.sort;
  const urlMinPrice = urlParams.minPrice;
  const urlMaxPrice = urlParams.maxPrice;
  const currentInStock = urlParams.inStock;
